...
def SVC_fit_predict(shadow_train, shadow_test, target_train, target_test):
    ...
    # ✅ Sanity check
    if not np.all(np.isfinite(X_shadow)):
        print("Detected NaN or Inf in X_shadow. Filtering...")
        mask = np.isfinite(X_shadow).all(axis=1)
        X_shadow = X_shadow[mask]
        Y_shadow = Y_shadow[mask]
    ...
    if n_target_train > 0:
        ...
        if not np.all(np.isfinite(X_target_train)):
            print("Cleaning target_train")
            X_target_train = X_target_train[np.isfinite(X_target_train).all(axis=1)]
        acc_train = clf.predict(X_target_train).mean()
        accs.append(acc_train)

    if n_target_test > 0:
        X_target_test = target_test.cpu().numpy().reshape(n_target_test, -1)
        if not np.all(np.isfinite(X_target_test)):
            print("Cleaning target_test")
            X_target_test = X_target_test[np.isfinite(X_target_test).all(axis=1)]
...
def SVC_MIA(shadow_train, target_train, target_test, shadow_test, model):
    ...
    print("shadow_train_prob:", shadow_train_prob.shape)
    print("shadow_train_labels:", shadow_train_labels.shape)

    print("shadow_test_prob:", shadow_test_prob.shape)
    print("shadow_test_labels:", shadow_test_labels.shape)

    print("target_train_prob:", target_train_prob.shape)
    print("target_train_labels:", target_train_labels.shape)

    print("target_test_prob:", target_test_prob.shape)
    print("target_test_labels:", target_test_labels.shape)
    ...
    m = {
        "correctness": acc_corr,    # corr: MIA accuracy based on prediction correctness
        "confidence": acc_conf,     # conf: MIA accuracy based on the confidence of the true label
        "entropy": acc_entr,
